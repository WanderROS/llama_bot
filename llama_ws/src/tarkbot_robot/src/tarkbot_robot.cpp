...
TarkbotRobot::TarkbotRobot()
{
    ...
    private_nh_.param<std::string>("laser_on_off_topic", laser_on_off_topic_, "laser_on_off");

    //参数初始化
    private_nh_.param<std::string>("robot_port", serial_port_, "/dev/ttyS0");
    private_nh_.param<int>        ("robot_port_baud", serial_port_baud_, 230400);
    private_nh_.param<bool>       ("pub_odom_tf", pub_odom_tf_, true);
    
    //车型选择参数
    private_nh_.param<std::string>("robot_type_send", robot_type_send_, "r10_twd1");
    ...
    laser_on_off_sub_ = nh_.subscribe<std_msgs::Int8>(laser_on_off_topic_, 10,&TarkbotRobot::laser_on_off_callback,this);
    ...
    if(robot_type_send_== "r10_akm")
    cartype_data[0] = 1;
    if(robot_type_send_== "r10_twd1")
    cartype_data[0] = 2;
...
/*
 * @功  能  激光雷达开关话题订阅回调函数，根据订阅的指令通过串口发指令控制下位机
 */
void TarkbotRobot::laser_on_off_callback(const std_msgs::Int8::ConstPtr& laser)
{
    static uint8_t laser_data[1];

    //数据转换
    laser_data[0] = (uint8_t)(laser->data);

    //发送串口数据
    sendSerialPacket(laser_data, 1, ID_ROS2CTR_LASER);
}
